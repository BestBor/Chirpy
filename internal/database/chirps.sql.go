...
const getAllChirpsByCreatedAt = `-- name: GetAllChirpsByCreatedAt :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
ORDER BY created_at
`

func (q *Queries) GetAllChirpsByCreatedAt(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsByCreatedAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
